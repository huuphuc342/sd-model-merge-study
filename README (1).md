...
- **4 reference checkpoints** evaluated per comparison: `duchaitenaiart_V453` (base) → `CUAMIX_V6` → `UnrealWorld_v1` → `UnrealWorld_v3`
...
**Base (`duchaitenaiart_V453`) → V6 → UnrealWorld_v1 → UnrealWorld_v3**
...
<!-- Columns: duchaitenaiart_V453 | CUAMIX_V6 | UnrealWorld_v1 | UnrealWorld_v3 -->
<!-- Recommended resolution: 512x768 or 768x1024 per column -->
<!-- Label each column with checkpoint name -->
```
[ INSERT: Full portrait — Base vs V6 vs UnrealWorld_v1 vs UnrealWorld_v3 ]
```

**Observation:** Progressive Weighted Sum iterations (V2→V6) improved anatomical coherence incrementally. The most visually distinct improvement occurred at the Add Difference stage (UnrealWorld v2→v3): contrast, facial depth-of-field, and skin micro-detail showed step-change improvement not achievable through ratio tuning alone.
...
```
[ INSERT: Face crop — Base vs V6 vs UnrealWorld_v1 vs UnrealWorld_v3 ]
```
...
```
[ INSERT: Eye crop — Base vs V6 vs UnrealWorld_v1 vs UnrealWorld_v3 ]
```
...
```
[ INSERT: Hand/finger crop — Base vs V6 vs UnrealWorld_v1 vs UnrealWorld_v3 ]
```
